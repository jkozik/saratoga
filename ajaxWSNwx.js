...
*/
  ...
  if (x) { // got something back
    ...
    try { if (x.readyState == 4 && x.status == 200) { // Mike Challis added fix to fix random error: NS_ERROR_NOT_AVAILABLE 
	  var realtime = JSON.parse(x.responseText, function (key, value) {
		var type;
	...
	if(endpattern.test(x.responseText) && 
		...
		rWindUOM = realtime.wind.speed['unit'];  // extract units-of-measure used in realtime file and replace defaults
		rTempUOM = realtime.temperature.outdoor['unit'];
		rBaroUOM = realtime.barometricPressure['unit'];
		rRainUOM = realtime.rain.day['unit'];
        
        //Temperature
		var temp = convertTemp(realtime.temperature.outdoor[rTempUOM]);
...
//		  ", " + langThermoMin + tempmin.toFixed(1) + uomTemp;
		...
		//Humidity ...
		var humidity = realtime.humidity.outdoor['%'];
		set_ajax_obs("ajaxhumidity",humidity);
		set_ajax_obs("gizmohumidity",humidity);
		
		//Dewpoint ...
		var dew = convertTemp(realtime.dewPoint.outdoor[rTempUOM]);
		set_ajax_obs("ajaxdew",dew.toFixed(1) + uomTemp);
		set_ajax_obs("gizmodew",dew.toFixed(1) + uomTemp);

		//  WindChill
		var windchill = convertTemp(realtime.windChill[rTempUOM]);
		set_ajax_obs("ajaxwindchill",windchill.toFixed(1) + uomTemp);

		// Heat Index
		var heatidx = convertTemp(realtime.heatIndex.outdoor[rTempUOM]);
		set_ajax_obs("ajaxheatidx",heatidx.toFixed(1) + uomTemp);
        // FeelsLike
        temp = realtime.temperature.outdoor.C; // note.. temp in C
        if (temp <= 16.0 ) {
		  feelslike = realtime.windChill.C; //use WindChill
		} else if (temp >=27.0) {
		  feelslike = realtime.heatIndex.outdoor.C; //use HeatIndex
		} else {
...
//		var heatColorWord = heatColor(realtime[2],realtime[24],realtime[41]); // FIX this
//		set_ajax_obs("ajaxheatcolorword",heatColorWord);
		
		//Pressure...
		var pressure = convertBaro(realtime.barometricPressure[rBaroUOM]);
		set_ajax_obs("ajaxbaro",pressure.toFixed(dpBaro) + uomBaro);
		set_ajax_obs("ajaxbaroNoU",pressure.toFixed(dpBaro));
		set_ajax_obs("gizmobaro",pressure.toFixed(dpBaro) + uomBaro);
		if(langTransLookup[realtime.barometricTrend['value']] == undefined) {
		...
		var barotrendtext = realtime.barometricTrend["value"];
		if(langTransLookup[barotrendtext] != undefined) {
...
*/
        //Wind gust
		var gust    = convertWind(realtime.wind.speed[rWindUOM]);
		var maxgust = convertWind(realtime.wind.gust[rWindUOM]);
		if (maxgust > 0.0 ) {
		...
		//Windspeed ...
		var wind = convertWind(realtime.wind.speed[rWindUOM]);
		var beaufortnum = ajax_get_beaufort_number(wind);
		set_ajax_obs("ajaxbeaufortnum",beaufortnum);
		set_ajax_obs("ajaxbeaufort",langBeaufort[beaufortnum]);

       //WIND DIRECTION ...
        var val = windDir(realtime.wind.direction.degrees);
		var valLang = windDirLang(realtime.wind.direction.degrees); /* to enable translations */
...
//		set_ajax_obs("ajaxwindmaxgusttime",windmaxgusttime);
		...
		//  Solar Radiation
		var solar    = realtime.solarRadiation['w/m2'] * 1.0;
		set_ajax_obs("ajaxsolar",solar.toFixed(0));

		// UV Index		
		var uv       = realtime.uvIndex['value'];
		...
		//Rain ...
		var rain = convertRain(realtime.rain.day[rRainUOM]);
...
*/
		var rainmo = convertRain(realtime.rain.month[rRainUOM]);
		set_ajax_obs("ajaxrainmo",rainmo.toFixed(dpRain) + uomRain);

		var rainyr = convertRain(realtime.rain.year[rRainUOM]);
		set_ajax_obs("ajaxrainyr",rainyr.toFixed(dpRain) + uomRain);

		var rainratehr = convertRain(realtime.rain.rate[rRainUOM+'/hr']); // make per hour rate.
...
*/
		...
		var ajaxdateraw = realtime.station['time']; // comes in '2012-01-13 11:25:01 -0800' format

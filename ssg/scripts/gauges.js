 ...
 *
 * Created by Mark Crossley, 14 February 2013
 *
 * File encoding = UTF-8
 *
 */
/*globals steelseries, LANG, changeLang, windRose*/
/*jshint jquery:true */
...
var gauges = (function () {
    var strings = LANG.EN,         //Set to your default language. Store all the strings in one object
        config = {
            ...
            weatherProgram    : 2,                      //Set 0=Cumulus, 1=Weather Display, 2=VWS, 3=WeatherCat
            imgPathURL        : '/vwsimages/',             //*** Change this to the relative path for your 'Trend' graph images
            ...
            realTimeURL_VWS    : 'ssg/steelseriesVWSjson.php', //*** VWS Users: Change to your location of the JSON script generator ***
        ...
        _led = {},
        _sampleDate,
        _realtimeVer = 9,   //minimum version of the realtime JSON file required
...
/*        _imgBackground,    // Uncomment if using a background image on the gauges */
        ...
        init = function () {
            ...
            default:
                ...
                config.tipImgs = [                     // config.tipImgs - unknown
                    [null, null],
...
*/
            ...
            _solar.maxValue = gauge.solarGaugeScaleMax;
            ...
            } else if (windRose !== "undefined") {
                windRose.init(Math.ceil($('#canvas_rose').width() * config.gaugeScaling),
                           config.showRoseGaugeOdo);
        ...
        drawRain = function () {
            ...
            if ($('#canvas_rain').length) {
                ...
                params.valueColor = gauge.pointerColour;
        ...
        drawSolar = function () {
            ...
            if ($('#canvas_solar').length) {
                ...
                params.maxValue = _solar.maxValue;
        ...
        processData = function (dataStr) {
            ...
            if (data.ver !== undefined && data.ver >= _realtimeVer) {
                ...
                //Check WeatherCat version >=1.14 required
                if (config.weatherProgram === 3) {
                    ...
                    if (+data.version < 1.14) {
                        alert('Version of WeatherCat must be 1.14 or later to use these gauges!');
                        return;
                    }
                }
        ...
        doRain = function () {
            ...
            if (data.rainunit === 'mm') { // 10, 20, 30...
                ...
                if (_rain.value < 6) {
                    _rain.maxValue = Math.max(Math.ceil(_rain.value), 1.0);
                } else {
        ...
        doRRate = function () {
            ...
            if (data.rainunit === 'mm') { // 10, 20, 30...
                _rrate.maxValue = Math.max(Math.ceil(_rrate.maxMeasured / 10) * 10, 10);
            } else {
                // inches 0.5, 1.0, 1.5, 2.0 ...
                //_rrate.maxValue = Math.max(Math.ceil(_rrate.maxMeasured * 2) * 0.5, 0.5);
                // inches 1.0, 2.0, 3.0 ...
                _rrate.maxValue = Math.max(Math.ceil(_rrate.maxMeasured), 1.0);
            }
        ...
        doUV = function () {
            ...
            if (+_uv.value === 0) {
                _uv.risk = strings.uv_levels[0];
                _uv.headLine = strings.uv_headlines[0];
                _uv.detail = strings.uv_details[0];
            } else if (_uv.value < 3) {
                _uv.risk = strings.uv_levels[1];
                _uv.headLine = strings.uv_headlines[1];
                _uv.detail = strings.uv_details[1];
            } else if (_uv.value < 6) {
                _uv.risk = strings.uv_levels[2];
                _uv.headLine = strings.uv_headlines[2];
                _uv.detail = strings.uv_details[2];
            } else if (_uv.value < 8) {
                _uv.risk = strings.uv_levels[3];
                _uv.headLine = strings.uv_headlines[3];
                _uv.detail = strings.uv_details[3];
            } else if (_uv.value < 11) {
                _uv.risk = strings.uv_levels[4];
                _uv.headLine = strings.uv_headlines[4];
                _uv.detail = strings.uv_details[4];
            } else {
                _uv.risk = strings.uv_levels[5];
                _uv.headLine = strings.uv_headlines[5];
                _uv.detail = strings.uv_details[5];
            }
            ...
            if (ddimgtooltip.showTips) {
                // update tooltip
                tip = '<b>' + strings.uv_title + ': ' + _uv.value + '</b><br>';
        ...
        doSolar = function () {
            ...
            if (ddimgtooltip.showTips) {
                ...
                tip = '<b>' + strings.solar_title + ': ' + _solar.value + ' W/m²</b><br>' +

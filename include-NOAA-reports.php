...
if(!$validWX) {
	print "<h2>NOAA reports not available as weather software is not yet specified.</h2>\n";
	return;
}
...
//if(!preg_match('|^\d{4}$|',$yr)) { $yr = ''; }
$mo =   isset($_REQUEST['mo']) ? $_REQUEST['mo'] : '';
//if(!preg_match('|^\d{2}$|',$mo)) { $mo = ''; }
...
//
 for ($y = $now_year; $y >= $first_year; $y--) {
   ...
   if (isset($filesfound[$t]) || $yy == $now_year) {
      echo "<a href=\"$PHP_SELF?yr=$yy\" class=\"noaa_rep_nav\"><b>$yy</b></a>:";
	} else {
   ...
   for ($m = 1; $m <= 12; $m++) {
	 ...
	 if (isset($filesfound[$testfile])) {
	   echo " <a href=\"$PHP_SELF?yr=$yy&amp;mo=$mm\" class=\"noaa_rep_nav\"><b>" . $months[$mm]. "</b></a>";
	   if ($Naming == 'CU') {
	   ...
	   if ($yy == $now_year && $mm == $now_month) {
	      echo  " <a href=\"$PHP_SELF\" class=\"noaa_rep_nav\"><b>" . $months[$mm] ."</b></a>";
	   } else {
...
$BIDI = $RTLlang?' style="unicode-bidi: bidi-override;direction: ltr;"':'';
 if (! $yr && ! $mo && $Naming != "CU") {  // special for 'current month'
 ...
 if ($yr == $now_year && ! $mo && $Naming != "CU") { // special for current year
